<div class="form-container"id="form-container" >
    <form id="registerform" action="request.php" onsubmit="//return validateForm()" method="post">
        <div class='input-text required'>
            ...
            <?php if($_GET["errname"]==1){echo '<p class="error">Invalid Name! </p>';} ?>
        ...
        <div class='input-text required'>
            ...
            <?php if($_GET["errlastname"]==1){echo '<p class="error">Invalid Last Name! </p>';} ?>
            <p class="error hidden">Invalid Last Name! Check that name does not contain special symbols(e.g. %, $, #. @ etc.)</p>
            <p class="error hidden">This field can not be empty</p>
        </div>
        <div class='input-text required'>
            <input type="text" name="email" id="email" placeholder="Email*"/>
            <?php if($_GET["erremail"]==1){echo '<p class="error">Invalid Email! </p>';} ?>
        ...
        <div class='input-text required'>
            ...
            <?php if($_GET["errpassword"]==1){echo '<p class="error">Invalid Password! </p>';} ?>
            <p class="error hidden">Invalid Password! Password should contain at least one digit, at least one lower case, at least one upper case, at least 6 from the mentioned characters</p>
            <p class="error hidden">This field can not be empty</p>
        </div>
